class CardEffect < ActiveRecord::Base
  ...
  H_NECESSARY_CONDITIONS = [
    ...
    ['道路建設', ["HAND.cards.size >= 1"]],
    ['貨幣', ["!HAND.empty?"]],
    ['建築', ["HAND.cards.size >= 2",
              "AC_CARDS.size == 5 && OTHERS.all? { |p| p.active_cards(@game).size < 5 }"]],
    ['運河建設', ["!HAND.empty? && !INFLUENCE.empty?"]],
    ['発酵', ["RES_COUNTS[Resource.woods] >= 2"]],
    ['医術', ["!INFLUENCE.empty? && !@game.turn_player.influence_for(@game).empty?"]],
    ['機械', ["turn_hand = @game.turn_player.hand_for(@game); max_age = turn_hand.max_age; HAND.cards.size > turn_hand.cards.find_all { |c| c.age.level == max_age }.size",
    ...
    ['教育', ["INFLUENCE.cards.size >= 2"]],
    ['錬金術', ["RES_COUNTS[Resource.stone] >= 3",
                "HAND.cards.size >= 2"]],
    ['翻訳', ["!INFLUENCE.empty?",
              "AC_CARDS.all? { |c| c.has_resource?('金属') }"]],
  ...
  H_EVALUATION_F = [
    ...
    ['神秘主義', ["(BOARDS.to_a.count { |b| !b.empty? } - 1) * 25 + 100"]],
    ...
    ['哲学', ["200",
              "((HAND.cards.size - 2) * 25 + 50) + ((HAND.max_age - 1) * 50 + 50)"]],
    ['地図作成', ["200"]],
    ['数学', ["(HAND.max_age - 1) * 100 + 100"]],
    ['暦', ["200"]],
    ['一神論', ["((AC_CARDS.find_all { |c| !@game.turn_player.active_colors(@game).include?(c.color) }.map(&:age).map(&:level).min || 0) - 1) * 100 + 100"]],
    ['道路建設', ["red_age = BOARD_red.active_card&.age&.level || 0; green_age = BOARD_green.active_card&.age&.level || 0; HAND.cards.size >= 2 && red_age > 0 && OTHERS.any? { |p| (p.active_cards(@game).detect { |c| c.color == Color.red }&.age&.level || 0) >= red_age && (p.active_cards(@game).detect { |c| c.color == Color.green }&.age&.level || 0) <= green_age } ? 200 : 100"]],
    ['貨幣', ["HAND.cards.map(&:age).map(&:level).uniq.size * 100 + 50"]],
    ['建築', ["200 - (@game.stocks.find_by(age: Age.find_by(level: 2)).empty? ? 50 : 0)",
              "CONQUEST"]],
    ['運河建設', ["((HAND.cards.map(&:age).map(&:level).find_all { |age| age == HAND.max_age }.sum - INFLUENCE.cards.map(&:age).map(&:level).find_all { |age| age == HAND.max_age }.sum) - 2) * 50 + 100"]],
    ['発酵', ["((RES_COUNTS[Resource.woods] / 2).floor - 1) * 100 + 100"]],
    ['医術', ["min_age = @game.turn_player.influence_for(@game).min_age; min_age.nil? ? 0 : (INFLUENCE.max_age - min_age) * 100 + 50"]],
    ['機械', ["turn_hand = @game.turn_player.hand_for(@game); max_age = turn_hand.max_age; ((HAND.cards.size - turn_hand.cards.find_all { |c| c.age.level == max_age }.size - 1) * 50 + 50) + ((max_age - 1) * -100 + 100)",
              "(((HAND.cards.find_all { |c| c.has_resource?('石') }.map(&:age).map(&:level).max || 0) - 1) * 100 + 100) + (BOARD_red&.expandable_left? ? 200 : 0)"]],
    ['封建主義', ["(HAND.cards.size - 1) * 50 + 100",
                  "200"]],
    ['方位磁石', ["((((AC_CARDS.find_all { |c| c.has_resource?('木') && c.color != Color.green }.map(&:age).map(&:level).min || 0) - (@game.turn_player.active_cards(@game).find_all { |c| c.color != Color.green }.map(&:age).map(&:level).max || 0)) - 1) * 100 + 50) + ((@game.turn_player.active_cards(@game).find_all { |c| !c.has_resource?('木') }.map(&:age).map(&:level).min || 0) * -50 + 100)"]],
    ['光学', ["(@game.stocks.find { |s| s.age.level == 3}.empty? ? 50 : 0) + (INFLUENCE.empty? || OTHERS.none? { |p| p.influence_point(@game) < @player.influence_point(@game) } ? 100 : (INFLUENCE.cards.map(&:age).map(&:level).min || 0) * -50 + 100)"]],
    ['工学', ["(AC_CARDS.find_all { |c| c.has_resource?('石') }.map(&:age).map(&:level).sum - 1) * 100 + 200",
              "200"]],
    ['教育', ["max_age = INFLUENCE.max_age || 0; next_max_age = INFLUENCE.cards.map(&:age).map(&:level).sort.reverse[1] || 0; ((max_age - 1) * -50 + 100) + ((next_max_age - 1) * 50 + 50)"]],
    ['錬金術', ["((HAND.cards + @game.players.flat_map { |p| p.boards_for(@game).flat_map(&:cards) }).count { |c| c.age.level == 4 && c.color == Color.red } * 150) + ((HAND.cards.size - 1) * -50 + 200) + (((RES_COUNTS[Resource.stone] / 3).floor - 1) * 50 + 100)",
                "(HAND.cards.map(&:age).map(&:level).sum - 4) * 50 + 100"]],
    ['翻訳', ["((INFLUENCE.max_age - @player.max_age_on_boards(@game) - 1) * 50 + 100) + ((INFLUENCE.cards.size - 1) * -50 + 100)",
              "CONQUEST"]],
    ['紙', ["200",
            "(BOARDS.to_a.count { |b| b.expanded_left? } - 1) * 100 + 200"]],
#!!!
    ...
    ['奴隷解放', ["(HAND.cards.size - 1) * 25 + 100",
                  "(([BOARD_red, BOARD_purple].find_all(&:expandable_right?).map { |b| b.cards.size }.max || 0) - 2) * 100 / 3 + 100"]],
    ['分類', ["(OTHERS.all? { |p| !p.hand_for(@game).empty? } ? 25 : 0) + ((HAND.cards.size - 2) * 25 + 50) + ((OTHERS.sum { |p| p.hand_for(@game).cards.size } - OTHERS.size * 2) * 25 + 25)"]],
    ...
    ['ロケット工学', ["([OTHERS.map { |p| p.influence_for(@game).cards.size }.max || 0, (RES_COUNTS[Resource.time] / 2).floor].min - 1) * 100 + 100"]],
    ...
    ['共産主義', ["h_cards = HAND.cards.group_by { |c| c.age.level <=> @player.max_age_on_boards(@game) }; cards = h_cards[-1] || []; (h_cards[1]&.size || 0) > 0 || cards.size.zero? ? 0 : ((cards.size - 1) * 100 / 3 + 100) + (cards.any? { |c| c.color == Color.purple } ? 50 : 0)"]],
    ...
    ['人工衛星', ["HAND.cards.size * -25 + 150",
                  "(BOARD_purple.cards.size - 2) * 100 / 3 + 200",
                  "((HAND.cards.map { |c| c.effects.to_a.count(&:is_for_all) }.max || 0) - 1) * 50 + 100"]],
    ...
    ['ソフトウェア', ["100",
                      "200"]],
    ['ホームオートメーション', ["AC_CARDS.find_all { |c| c.title != 'ホームオートメーション' && c.effects.any? { |e| e.is_for_all } }.map { |c| c.effect_factor_sum(self) }.max || 0"]],
